...
- The agenda's date-window model is unified across day / week /
  ...
  [ADR-0028](docs/adr/0028-week-start-and-the-month-grid.md) for
  `--week-start` and the grid scope, and
  [ADR-0030](docs/adr/0030-explicit-window-in-the-month-grid.md) for
  why an explicit window in `month-grid` comes back grown to whole
  weeks. A reviewer finding "the grid returned more days than asked
  for" closes with a pointer to ADR-0030.
...
- JSON output evolution follows
  ...
  breaking and bumps the version. The crate's public Rust names are a
  second contract, and the two do not move together: a field added to
  `pub struct Task` is additive for the JSON and breaking for an
  embedder that builds the struct with a literal (ADR-0025). Say both
  in the CHANGELOG entry — the JSON rule and what the embedder has to
  change — rather than the JSON one alone.
...
- `timestamp_next` (agenda `day`/`week`/`month` only) holds a repeating
  ...
  carries it. Since ADR-0031 it names the next occurrence the series
  actually has: a cancelled or replaced one is stepped over. A reviewer
  finding "the field disagrees with this cell's date" or "tasks mode is
  missing the field" closes with a pointer to it.
- `timestamp_next_after` answers the other question — which occurrence
  follows the day this cell is drawn on. See
  [ADR-0029](docs/adr/0029-next-occurrence-after-the-rendered-day.md):
  it is filled only on the `scheduled_*` buckets of a dated scope,
  never on `overdue` / `upcoming`, which keep answering from now, and
  only where `timestamp_next` was computed — the Markdown and HTML
  renderers ask for neither and pay for neither. It steps over the
  occurrences the series does not have, the same way `timestamp_next`
  does. A reviewer finding "the field is missing from overdue" closes
  with a pointer to it.
- One occurrence of a repeating entry can be cancelled or moved without
  breaking the series apart, through three `org-properties` keys. See
  [ADR-0031](docs/adr/0031-exceptions-to-a-repeating-entry.md): `EXDATE`
  on the series cancels a date, and a separate entry carrying
  `SERIES_ID` + `RECURRENCE_ID` takes an occurrence's place — the shape
  RFC 5545 uses, matched by day. An exception reaches only as far as the
  scan does, and every way one can fail to work is reported through the
  `org-properties` diagnostic channel rather than passed over. ADR-0032
  answers what each of the two owes: a cancelled occurrence never was,
  so the arrears walk continues past it; a replaced one took place
  elsewhere, so the walk ends there. A reviewer finding "the series
  still draws the day it was moved from", "half a pair is accepted
  silently", or "the replacement is a second task" closes with a
  pointer to them.
- Third-party licence notices ship inside the release archives as
  `THIRD-PARTY-LICENSES.txt`, generated by
  `scripts/generate-third-party-licenses.sh` and committed. See
  [ADR-0024](docs/adr/0024-third-party-license-notices-in-archives.md):
  regenerate and commit the file with every dependency change (CI
  fails on a stale copy), keep the allow-list in `deny.toml` narrow,
  and treat a crate that ships no licence text as a decision to make,
  not a file to skip.
