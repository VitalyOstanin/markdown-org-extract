...
# stem; inside it: the binary, README.md, LICENSE and
# THIRD-PARTY-LICENSES.txt. See README "For downstream packagers" for the
# layout the workflow defends.
#
# Required env:
...
# third-party notices. The notice file is committed (generated by
# scripts/generate-third-party-licenses.sh, freshness enforced in CI) rather
# than produced here: packaging runs on all three runner OSes, while
# generation needs cargo plus the unpacked crate sources.
stage="${RUNNER_TEMP}/${stem}"
rm -rf "$stage"
mkdir -p "$stage"
cp "$BIN_PATH" "${stage}/${BIN_NAME}"
cp README.md LICENSE THIRD-PARTY-LICENSES.txt "$stage/"
